...
const calculateScenarios = (results) => {
  try {
    ...
    const shortfall = Math.abs(surplusCash);

    // Calculate additional annual investment required
    const r = returnRate / 100;
    const additionalAnnualInvestment = shortfall / ((Math.pow(1 + r, yearsToRetirement) - 1) / r);
    ...
    const currentFV = annualInvestment * ((Math.pow(1 + r, yearsToRetirement) - 1) / r);
    const additionalYears = currentFV > 0 ? Math.ceil(
      Math.log((currentFV + shortfall) / currentFV) / Math.log(1 + r)
    ) - yearsToRetirement : 0;

    return {
      additionalAnnualInvestment: Math.max(0, Math.round(additionalAnnualInvestment)),
